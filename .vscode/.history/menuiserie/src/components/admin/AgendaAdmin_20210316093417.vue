...
export default {
  ...
  methods: {
    async getEvents() {
      ...
      const item = response.data.map((item) => item);

      eventsTab.push({
        description: item.description,
        endDate: this.$moment(item.endDate).format(format1),
        startDate: this.$moment(item.startDate).format(format1),
        text: item.text,
      });
      this.events = eventsTab;
