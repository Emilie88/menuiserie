...
export default {
  ...
  methods: {
    async getEvents() {
      ...
      let eventTab = response.data.map((item) => {
        (item.start = moment(item.start).format("YYYY-MM-DDThh:mm")),
          (item.end = moment(item.end).format("YYYY-MM-DDThh:mm"));
      });
      this.events = eventTab;
