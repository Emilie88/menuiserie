...
export default {
  ...
  data: () => ({
    ...
    body: {
      name: null,
      start: moment("2021/03/16 16:30").format("YYYY-MM-DDThh:mm"),
      end: moment("2021/03/16 17:30").format("YYYY-MM-DDThh:mm"),
      color: null,
    },
  ...
  created() {
    this.getEvents();
    // console.log(moment("2021/03/16 17:30").format("YYYY-MM-DDThh:mm"));
  },
  ...
  methods: {
    async getEvents() {
      ...
      let eventTab = response.data.map(
        (item) => `${(item.name, item.color, item.start, item.end)}`
      );
      // this.events.push(
      //   eventTab.color,
      //   moment(eventTab.start).format("YYYY-MM-DDThh:mm"),
      //   moment(eventTab.end).format("YYYY-MM-DDThh:mm"),
      //   eventTab.name
      // );
